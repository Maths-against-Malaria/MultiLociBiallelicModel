...
MLE <- function(X, id = TRUE){
    ...
    X <- as.matrix(X, ncol=nloci)%*%trin + 1
    X <- t(as.data.frame(summary.factor(X)))    # Observations present in the dataset
    vals <- as.integer(colnames(X))-1           
    ...
    # MLEs
    out <- EstnbialModel(Nx, dat)
    out2 <- out[[2]]

    cnames <- c()
    for(i in 1:nHapl){
        cnames[i] <- paste(H[i,], collapse = '')
    }
    colnames(out2) <- cnames
    list(unlist(out[1]), out2)
}
